...
int CLogger::LogMessage(const char* fmt, ...)
{
    ...
    time(&long_time);                                  // Get time as 64-bit integer.
    tm = localtime(&long_time); 
    sprintf(dtbuf, "[%02d-%3s-%04d:%02d%02d] ", tm->tm_mday, lpszMonths[tm->tm_mon], 1900 + tm->tm_year, tm->tm_hour, tm->tm_min);
    ...
    int strlen = vsnprintf(nullptr, 0, fmt, vacopy);    // get space required for substitution
